...
run_sim <- function(nsims){
  ...
  #    df$turnover_home = df$turnover_home + df$freethrowa_home
    ...
    #  home_outcome = df$which_outcome_home
      ...
      #  player_outcome = sample_n(home_on_court, 1, replace = FALSE, weight = `3pa_home`)
      #  player_stat = home_team[home_team$player %in% home_on_court,]
       # player_stat = `player_stat`[,22]
        
        # home_outcome = replicate(1, sample(home_on_court, 1,prob= choose_path_start, replace = FALSE))
        
        
        #player_outcome = player_outcome %>% dplyr::select(player, `3p%`)
        
        player_stat = home_team[home_team$player %in% player_outcome,]
        player_stat = `player_stat`[,31]
        rand = runif(1, 0, 1)
        outcome = ifelse(player_stat > rand, "3P", "3P Miss") ##normally 1 is rand
        m = c(outcome,m)
        p = c(player_outcome, p)
        ...
        ##### vectoring done up till here (i think) #####
